...
- [ ] *(watch)* hardcoded shipping-rate table (`shipping.ts`) can drift from real Posten pricing — bounded (locked per listing) but a margin/support risk. **Drift guard added**: `SHIPPING_RATES_VERIFIED_ON` + a `shipping.test.ts` freshness check that fails once the rates are >18 months old, so drift surfaces in CI instead of silently. Re-verify against posten.no/priser and bump the date when it fires.
...
- [x] **Brønnøysund resilience / CI** — the create/lookup path now has a CI gate: `stores.test.ts` mocks `../brreg` + `../notify` and drives `createStore` end-to-end over fake-db (happy path + owner membership, non-normal status, duplicate orgnr, not_found, bad email pre-check, taken slug). And the live lookup is now **bounded** — both brreg fetches use `AbortSignal.timeout(8000)`, so a hung Brønnøysund returns `network_error` instead of stalling store creation.
...
**Frontend / perf / UX**
- [x] **ListingCard images** — `loading="lazy"` + `decoding="async"` + intrinsic `width`/`height` **and** real server-side resizing. Confirmed Supabase image transformations are enabled on the project (the render endpoint processes transforms, returning "Object not found" for a missing file rather than a "feature disabled" error), so `resizedImageUrl()` rewrites card image URLs to the render endpoint with `width`+`quality`, and ListingCard serves a retina `srcset` (400w/800w) + `sizes`. A 24-card grid now pulls small optimised thumbnails instead of two dozen full-res uploads. *(Studio's ProjectCard can adopt the same helper — pending the Strikkestua polish pass.)*
