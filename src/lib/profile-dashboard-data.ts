...
import { parseStored } from './services/dashboard-layout';
...
export async function loadProfileDashboard(supabase: SupabaseClient, user: DashUser) {
  ...
  // Saved dashboard arrangement (order + widget sizes + grid/masonry mode). RLS
  // scopes this to the owner; empty means "server default". Rendered into data
  // attributes so the edit controller can apply it without a client round-trip.
