...
export async function checkAndGrantAchievements(admin: SupabaseClient, userId: string, env?: Record<string, string | undefined>): Promise<string[]> {
  const { data: sellerConnect } = await admin
  ...
  const [
    ...
    admin.from('profiles')
      .select('created_at, avatar_path, bio, location, instagram_handle, trust_tier, role')
      .eq('id', userId).single(),
  ...
  if (sellerConnect?.stripe_connect_status === 'verified') await grant('stripe_onboarded');
