...
export const SCENARIOS: Record<string, Scenario> = {
  ...
  'commission-dispute': {
    title: 'Oppdrag → tvist → admin frigir',
    desc: 'Betalt oppdrag → ferdig → kjøper åpner tvist → admin frigir betalingen til strikker.',
    track: 'request',
    personas: ['liv', 'eline', 'nora'],
    steps: [
      { id: 'admin', actor: 'nora', action: 'set-role', label: 'Nora = admin', params: { role: 'admin' } },
      ...
      { id: 'create-request', actor: 'liv', action: 'create-request', label: 'Liv oppretter forespørsel',
        params: { title: 'Kabelgenser str 3 år', category: 'genser', size_label: '3 år', budget_nok_min: 900, budget_nok_max: 1600, description: 'Kabelgenser.' },
        expect: (s) => [['open', s.request?.status, 'open']] },
      { id: 'offer', actor: 'eline', action: 'make-offer', label: 'Eline gir tilbud',
        params: { request_id: '$create-request.id', price_nok: 1300, turnaround_weeks: 3, message: 'Klart!' },
        expect: (s) => [['offer pending', s.offers?.[0]?.status, 'pending']] },
      ...
      { id: 'complete', actor: 'eline', action: 'mark-completed', label: 'Eline merker ferdig',
        params: { request_id: '$create-request.id', tracking_code: 'POSTEN-COMM-D' },
        expect: (s) => [['completed', s.request?.status, 'completed']] },
      { id: 'dispute', actor: 'liv', action: 'dispute-commission', label: 'Liv åpner tvist',
        params: { request_id: '$create-request.id', reason: 'Genseren kom aldri fram.' },
        expect: (s) => [
          ['disputed', s.request?.status, 'disputed'],
          ['ledger: dispute_opened', has(s.payment_events, 'dispute_opened'), true],
        ] },
      { id: 'resolve', actor: 'nora', action: 'resolve-dispute', label: 'Nora frigir til strikker',
        params: { item_type: 'commission', item_id: '$create-request.id', decision: 'release', notes: 'Sporing viser levering.' },
        expect: (s) => [
          ['ledger: dispute_resolved', has(s.payment_events, 'dispute_resolved'), true],
          ['ledger: released', has(s.payment_events, 'released'), true],
        ] },
    ],
  },

  'commission-cancel-late': {
    title: 'Oppdrag → frist utløpt → refusjon',
    desc: 'Betalt oppdrag → strikker leverer ikke innen frist + frist-grace → kjøper kansellerer → penger refundert.',
    track: 'request',
    personas: ['liv', 'eline'],
    steps: [
      { id: 'verify-knitter', actor: 'eline', action: 'set-stripe-onboarded', label: 'Eline verifisert' },
      { id: 'create-request', actor: 'liv', action: 'create-request', label: 'Liv oppretter forespørsel med frist',
        params: { title: 'Dåpskjole', category: 'kjole', size_label: '0-3 mnd', budget_nok_min: 1200, budget_nok_max: 2200, description: 'Til dåp.', needed_by: '2026-01-15' },
        expect: (s) => [['open', s.request?.status, 'open']] },
      { id: 'offer', actor: 'eline', action: 'make-offer', label: 'Eline gir tilbud',
        params: { request_id: '$create-request.id', price_nok: 1800, turnaround_weeks: 4, message: 'Jeg kan!' },
        expect: (s) => [['offer pending', s.offers?.[0]?.status, 'pending']] },
      { id: 'accept', actor: 'liv', action: 'accept-offer', label: 'Liv aksepterer', params: { offer_id: '$offer.id' },
        expect: (s) => [['awaiting_payment', s.request?.status, 'awaiting_payment']] },
      { id: 'pay', actor: 'liv', action: 'pay', label: 'Liv betaler', params: { request_id: '$create-request.id' },
        expect: (s) => [['awarded', s.request?.status, 'awarded']] },
      { id: 'cancel-late', actor: 'liv', action: 'cancel-late-commission', label: 'Liv kansellerer etter utløpt frist',
        params: { request_id: '$create-request.id', now: '2026-03-01' },
        expect: (s) => [
          ['cancelled', s.request?.status, 'cancelled'],
          ['ledger: refunded', has(s.payment_events, 'refunded'), true],
        ] },
    ],
  },

  'commission-auto-release': {
    title: 'Oppdrag → auto-frigivelse (cron)',
    desc: 'Betalt oppdrag → ferdig → kjøper bekrefter ikke → cron frigir betalingen til strikker etter leveringsvinduet.',
    track: 'request',
    personas: ['liv', 'eline'],
    steps: [
      { id: 'verify-knitter', actor: 'eline', action: 'set-stripe-onboarded', label: 'Eline verifisert' },
      { id: 'create-request', actor: 'liv', action: 'create-request', label: 'Liv oppretter forespørsel',
        params: { title: 'Lue og votter', category: 'lue', size_label: '2 år', budget_nok_min: 400, budget_nok_max: 900, description: 'Sett.' },
        expect: (s) => [['open', s.request?.status, 'open']] },
      { id: 'offer', actor: 'eline', action: 'make-offer', label: 'Eline gir tilbud',
        params: { request_id: '$create-request.id', price_nok: 700, turnaround_weeks: 2, message: 'Klart!' },
        expect: (s) => [['offer pending', s.offers?.[0]?.status, 'pending']] },
      { id: 'accept', actor: 'liv', action: 'accept-offer', label: 'Liv aksepterer', params: { offer_id: '$offer.id' },
        expect: (s) => [['awaiting_payment', s.request?.status, 'awaiting_payment']] },
      { id: 'pay', actor: 'liv', action: 'pay', label: 'Liv betaler', params: { request_id: '$create-request.id' },
        expect: (s) => [['awarded', s.request?.status, 'awarded']] },
      { id: 'complete', actor: 'eline', action: 'mark-completed', label: 'Eline merker ferdig',
        params: { request_id: '$create-request.id', tracking_code: 'POSTEN-COMM-AR' },
        expect: (s) => [['completed', s.request?.status, 'completed']] },
      { id: 'cron-release', actor: 'eline', action: 'release-commission', label: 'Cron frigir etter leveringsvinduet',
        params: { request_id: '$create-request.id' },
        expect: (s) => [
          ['ledger: released', has(s.payment_events, 'released'), true],
        ] },
  ...
  'listing-refund': {
    title: 'Refusjon (selger godtar)',
    desc: 'Kjøp → sendt → kjøper ber om refusjon → selger godtar → penger tilbake, annonse reaktiveres.',
    track: 'listing',
    personas: ['eline', 'liv'],
    steps: [
      ...
      { id: 'trust-seller', actor: 'eline', action: 'set-trust', label: 'Eline betrodd', params: { trust_tier: 'trusted', trust_score: 100 } },
      { id: 'create-listing', actor: 'eline', action: 'create-listing', label: 'Eline oppretter annonse',
        params: { title: 'Strikket jakke str 4 år', kind: 'pre_loved', category: 'cardigan', size_label: '4 år', price_nok: 399, condition: 'lite_brukt' },
        expect: (s) => [['draft', s.listing?.status, 'draft']] },
      { id: 'publish', actor: 'eline', action: 'publish-listing', label: 'Publiser', params: { listing_id: '$create-listing.id' },
        expect: (s) => [['active', s.listing?.status, 'active']] },
      { id: 'purchase', actor: 'liv', action: 'purchase-listing', label: 'Liv kjøper',
        params: { listing_id: '$create-listing.id', buyer_name: 'Liv Johansen', buyer_address: 'Storgata 12', buyer_postal_code: '0155', buyer_city: 'Oslo' },
        expect: (s) => [['reserved', s.order?.status, 'reserved']] },
      { id: 'ship', actor: 'eline', action: 'ship-listing', label: 'Eline sender', params: { listing_id: '$create-listing.id', tracking_code: 'POSTEN-55555' },
        expect: (s) => [['shipped', s.order?.status, 'shipped']] },
      { id: 'request-refund', actor: 'liv', action: 'request-refund', label: 'Liv ber om refusjon',
        params: { listing_id: '$create-listing.id', reason: 'not_as_described', description: 'Størrelsen stemmer ikke.' },
        expect: (s) => [['refund requested', !!s.order?.refund_requested_at, true]] },
      { id: 'accept-refund', actor: 'eline', action: 'respond-refund', label: 'Eline godtar refusjon',
        params: { listing_id: '$create-listing.id', refund_action: 'accept', notes: 'Beklager, refunderer.' },
        expect: (s) => [
          ['order cancelled', s.order?.status, 'cancelled'],
          ['refund accepted', s.order?.refund_outcome, 'accepted'],
          ['listing reactivated', s.listing?.status, 'active'],
          ['ledger: refunded', has(s.payment_events, 'refunded'), true],
        ] },
    ],
  },
  ...
  'listing-dispute': {
    title: 'Tvist → admin frigir',
    desc: 'Kjøp → sendt → kjøper åpner tvist → admin frigir betaling til selger.',
    track: 'listing',
    personas: ['eline', 'liv', 'nora'],
    steps: [
      { id: 'admin', actor: 'nora', action: 'set-role', label: 'Nora = admin', params: { role: 'admin' } },
      { id: 'verify-seller', actor: 'eline', action: 'set-stripe-onboarded', label: 'Eline verifisert' },
      { id: 'trust-seller', actor: 'eline', action: 'set-trust', label: 'Eline betrodd', params: { trust_tier: 'trusted', trust_score: 100 } },
      { id: 'create-listing', actor: 'eline', action: 'create-listing', label: 'Eline oppretter annonse',
        params: { title: 'Strikket pledd', kind: 'ready_made', category: 'teppe', size_label: '100x120', price_nok: 899 },
        expect: (s) => [['draft', s.listing?.status, 'draft']] },
      { id: 'publish', actor: 'eline', action: 'publish-listing', label: 'Publiser', params: { listing_id: '$create-listing.id' },
        expect: (s) => [['active', s.listing?.status, 'active']] },
      { id: 'purchase', actor: 'liv', action: 'purchase-listing', label: 'Liv kjøper',
        params: { listing_id: '$create-listing.id', buyer_name: 'Liv Johansen', buyer_address: 'Storgata 12', buyer_postal_code: '0155', buyer_city: 'Oslo' },
        expect: (s) => [['reserved', s.order?.status, 'reserved']] },
      { id: 'ship', actor: 'eline', action: 'ship-listing', label: 'Eline sender', params: { listing_id: '$create-listing.id', tracking_code: 'POSTEN-77777' },
        expect: (s) => [['shipped', s.order?.status, 'shipped']] },
      { id: 'dispute', actor: 'liv', action: 'dispute-listing', label: 'Liv åpner tvist',
        params: { listing_id: '$create-listing.id', reason: 'Pleddet var skadet ved mottak.' },
        expect: (s) => [
          ['order disputed', s.order?.status, 'disputed'],
          ['listing disputed', s.listing?.status, 'disputed'],
          ['ledger: dispute_opened', has(s.payment_events, 'dispute_opened'), true],
        ] },
      { id: 'resolve', actor: 'nora', action: 'resolve-dispute', label: 'Nora frigir til selger',
        params: { item_type: 'listing', item_id: '$create-listing.id', decision: 'release', notes: 'Selger sannsynliggjorde forsvarlig sending.' },
        expect: (s) => [
  ...
  'listing-ship-deadline': {
    title: 'Frist for sending utløper (cron)',
    desc: 'Kjøp → selger sender ikke i tide → cron frigir reservasjonen, annonsen relistes, kjøper ikke belastet.',
    track: 'listing',
    personas: ['eline', 'liv'],
    steps: [
      { id: 'verify-seller', actor: 'eline', action: 'set-stripe-onboarded', label: 'Eline verifisert' },
      { id: 'trust-seller', actor: 'eline', action: 'set-trust', label: 'Eline betrodd', params: { trust_tier: 'trusted', trust_score: 100 } },
      { id: 'create-listing', actor: 'eline', action: 'create-listing', label: 'Eline oppretter annonse',
        params: { title: 'Strikket topp str 5 år', kind: 'pre_loved', category: 'genser', size_label: '5 år', price_nok: 199 },
        expect: (s) => [['draft', s.listing?.status, 'draft']] },
      { id: 'publish', actor: 'eline', action: 'publish-listing', label: 'Publiser', params: { listing_id: '$create-listing.id' },
        expect: (s) => [['active', s.listing?.status, 'active']] },
      { id: 'purchase', actor: 'liv', action: 'purchase-listing', label: 'Liv kjøper',
        ...
        expect: (s) => [['reserved', s.order?.status, 'reserved']] },
      { id: 'cron-release', actor: 'eline', action: 'release-reservation', label: 'Cron frigir (ikke sendt i tide)',
        params: { listing_id: '$create-listing.id', reason: 'ship_deadline' },
        expect: (s) => [
          ['order cancelled', s.order?.status, 'cancelled'],
          ['reason ship_deadline', s.order?.cancel_reason, 'ship_deadline'],
          ['listing relisted', s.listing?.status, 'active'],
          ['buyer cleared', s.listing?.buyer_id, null],
          ['ledger: cancelled', has(s.payment_events, 'cancelled'), true],
        ] },
    ],
  },

  'listing-chargeback': {
    title: 'Betalingsinnsigelse (chargeback)',
    desc: 'Kjøp → sendt → banken åpner innsigelse (fryser ordren) → innsigelsen avgjøres.',
    track: 'listing',
    personas: ['eline', 'liv'],
    steps: [
      { id: 'verify-seller', actor: 'eline', action: 'set-stripe-onboarded', label: 'Eline verifisert' },
      { id: 'trust-seller', actor: 'eline', action: 'set-trust', label: 'Eline betrodd', params: { trust_tier: 'trusted', trust_score: 100 } },
      { id: 'create-listing', actor: 'eline', action: 'create-listing', label: 'Eline oppretter annonse',
        params: { title: 'Strikket kåpe', kind: 'ready_made', category: 'cardigan', size_label: 'S', price_nok: 1299 },
        expect: (s) => [['draft', s.listing?.status, 'draft']] },
      { id: 'publish', actor: 'eline', action: 'publish-listing', label: 'Publiser', params: { listing_id: '$create-listing.id' },
        expect: (s) => [['active', s.listing?.status, 'active']] },
      { id: 'purchase', actor: 'liv', action: 'purchase-listing', label: 'Liv kjøper',
        params: { listing_id: '$create-listing.id', buyer_name: 'Liv', buyer_address: 'Storgata 12', buyer_postal_code: '0155', buyer_city: 'Oslo' },
        expect: (s) => [['reserved', s.order?.status, 'reserved']] },
      { id: 'ship', actor: 'eline', action: 'ship-listing', label: 'Eline sender', params: { listing_id: '$create-listing.id', tracking_code: 'POSTEN-33333' },
        expect: (s) => [['shipped', s.order?.status, 'shipped']] },
      { id: 'chargeback-open', actor: 'eline', action: 'chargeback-open', label: 'Banken åpner innsigelse (fryser)',
        params: { listing_id: '$create-listing.id', reason: 'fraudulent' },
        expect: (s) => [
          ['order frozen', s.order?.status, 'disputed'],
          ['dispute id recorded', !!s.order?.stripe_dispute_id, true],
          ['listing disputed', s.listing?.status, 'disputed'],
          ['ledger: dispute_opened', has(s.payment_events, 'dispute_opened'), true],
        ] },
      { id: 'chargeback-close', actor: 'eline', action: 'chargeback-close', label: 'Innsigelsen avgjøres (vunnet)',
        params: { dispute_id: '$chargeback-open.dispute_id', outcome: 'won' },
        expect: (s) => [
          ['resolution recorded', s.order?.dispute_resolution, 'stripe_chargeback_won'],
          ['resolved_at set', !!s.order?.dispute_resolved_at, true],
          ['ledger: dispute_resolved', has(s.payment_events, 'dispute_resolved'), true],
        ] },
    ],
  },

  // NOTE: the "dead auth at ship" guard (shipListing must refuse to ship
  // against a canceled PaymentIntent and release the reservation) is covered by
  // the mutation-gated unit test in listings-money.test.ts — it's hard to
  // reproduce faithfully through the HTTP sim (the double's PI state can't be
  // steered across requests without leaking prod-shaped hacks), so it lives at
  // the unit layer, not here.
