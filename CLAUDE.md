...
>
> **Mutation gate:** `npm run test:mutation` runs Stryker over the money-critical code (`listings-escrow.ts` escrow paths, `commissions.ts` payment core, and the `money.ts` authority) and **breaks below 90% mutation score**. Run it after changing fee math, escrow, refund/payout, or `money.ts`. The break was 92 before the fee math was extracted into `src/lib/money.ts` — now all fee/conservation math lives there (mutation-tested at ~94%, plus `money-boundary.test.ts` + 111 invariant sweeps), so the service ranges are pure orchestration with defensive dead-letter branches; 90 is the calibrated floor for that structure (`money.ts` + the commissions money-core each hold ~94%). It is a **manual/local** gate (not run in CI); the `stryker.config.json` line-ranges drift when you add/remove lines in those files — re-derive them (grep the function boundaries) if the score craters with `no-cov` mutants. Config: `stryker.config.json`.
